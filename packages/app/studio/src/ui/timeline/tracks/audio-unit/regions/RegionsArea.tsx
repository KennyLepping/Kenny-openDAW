...
import {
  RegionCaptureTarget,
  RegionCapturing,
} from "@/ui/timeline/tracks/audio-unit/regions/RegionCapturing.ts";
...
export const RegionsArea = ({
  ...
  lifecycle.ownAll(
    regionSelection.catchupAndSubscribe({
      ...
      onDeselected: (selectable: AnyRegionBoxAdapter) =>
    ...
    shortcuts.register(RegionsShortcuts["select-all"].shortcut, () => {
      regionSelection.select(
        ...manager
          .tracks()
          .flatMap(({ trackBoxAdapter: { regions } }) =>
            regions.collection.asArray(),
          ),
      );
    }),
    shortcuts.register(RegionsShortcuts["deselect-all"].shortcut, () =>
    ...
    shortcuts.register(RegionsShortcuts["toggle-mute"].shortcut, () => {
      ...
      editing.modify(() =>
        selected.forEach(({ box: { mute } }) => mute.toggle()),
      );
      return true;
    }),
    ...
    Events.subscribeDblDwn(element, (event) => {
      ...
      if (target?.type === "region") {
        ...
        position = Math.max(
          position,
          trackBoxAdapter.regions.collection.lowerEqual(position)?.complete ??
            Number.NEGATIVE_INFINITY,
        );
        complete = Math.min(
          complete,
          trackBoxAdapter.regions.collection.greaterEqual(position + 1)
      ...
      yAxis={{
        axisToValue: (y) =>
          clamp(
            ...
            0,
            scrollContainer.scrollTop + element.scrollHeight,
          ),
  ...
  lifecycle.ownAll(
    installAutoScroll(
      ...
      (deltaX, deltaY) => {
        ...
        if (deltaX !== 0) {
          range.moveUnitBy(
            deltaX * range.unitsPerPixel * Config.AutoScrollHorizontalSpeed,
          );
        }
      },
      {
    ...
    Events.subscribe(
      ...
      (event: WheelEvent) => {
        if (event.shiftKey) {
          ...
          const threshold = 5.0;
          const clamped =
            Math.max(deltaX - threshold, 0.0) +
            Math.min(deltaX + threshold, 0.0);
    ...
    installCursor(element, capturing, {
      get: (target: Nullable<RegionCaptureTarget>, event: CursorEvent) => {
        const units = snapping.xToUnitRound(
        ...
        markerPosition.setValue(
          event.altKey &&
            target !== null &&
            target.type === "region" &&
            target.region.position < units &&
            units < target.region.complete
            ? units
            : null,
        );
        ...
        switch (target.part) {
          case "start":
            return manager.startRegionModifier(
              RegionStartModifier.create(regionSelection.selected(), {
                project,
                element,
                snapping,
                pointerPulse,
          ...
          case "complete":
            return manager.startRegionModifier(
              RegionDurationModifier.create(regionSelection.selected(), {
                project,
                element,
                snapping,
                pointerPulse,
          ...
          case "content-complete":
            return manager.startRegionModifier(
              RegionLoopDurationModifier.create(regionSelection.selected(), {
                project,
                element,
                snapping,
                pointerPulse,
            ...
            if (event.shiftKey) {
              ...
              return Option.wrap({
                update: (dragEvent: Dragging.Event) => {
                  ...
                  const newSlope = clamp(
                    originalSlope + deltaY * 0.01,
                    0.001,
                    0.999,
                  );
            ...
            return Option.wrap({
              update: (dragEvent: Dragging.Event) => {
                const pointerPpqn = range.xToUnit(
                  dragEvent.clientX - clientRect.left,
                );
                editing.modify(() => {
                  if (isFadeIn) {
                    const newFadeIn = clamp(
                      pointerPpqn - position,
                      0,
                      duration,
                    );
                    ...
                    const newFadeOut = clamp(
                      complete - pointerPpqn,
                      0,
                      duration,
                    );
