...
export type RegionCaptureTarget =
  | { type: "region"; part: "gen"; region: AudioRegionBoxAdapter } // NEW
...
const GEN_HIT_LEFT_PAD_PX = 0; // local to region start; keep permissive
const GEN_HIT_WIDTH_PX = 64;   // generous: icon + "GEN" + divider + padding
const GEN_HIT_HEIGHT_PX = RegionLabel.labelHeight();
  ...
  export const create = (element: Element, manager: TracksManager, range: TimelineRange) =>
    new ElementCapturing<RegionCaptureTarget>(element, {
      capture: (x: number, y: number): Nullable<RegionCaptureTarget> => {
        ...
        const trackIndex = BinarySearch.rightMostMapped(
          tracks,
          y,
          NumberComparator,
        ...
        for (const region of collection.iterateFrom(position - threshold)) {
          ...
          // --- NEW: GEN capture (do this early so it wins over edge drag) ---
          if (isInstanceOf(region, AudioRegionBoxAdapter)) {
            const inRegionX = x >= x0 && x <= x1;
            const localY = y - track.position;
            const inLabelBand = localY >= 0 && localY <= GEN_HIT_HEIGHT_PX;

            if (inRegionX && inLabelBand) {
              const localX = x - x0; // local to region start in CSS px
              const inGenX =
                localX >= GEN_HIT_LEFT_PAD_PX &&
                localX <= GEN_HIT_LEFT_PAD_PX + GEN_HIT_WIDTH_PX;

              if (inGenX) {
                return { type: "region", part: "gen", region };
              }
            }

            // existing fading handles
            const { fading } = region;
